...
const path = require("path"); // ✅ Added for vercel path handling
...
// ✅ Use absolute path for db.json (important for Vercel)
...
app.get("/adventures", (req, res) => {
  ...
  if (!cityId) {
    return res.status(400).send({ message: "City ID not provided!" });
  }

  let city = data.find((item) => item.id == cityId);

  if (city && city.adventures) {
    return res.json(city.adventures);
  } else {
    return res.status(400).send({
      message: `Adventure not found for ${cityId}!`,
    });
  }
});
...
app.post("/reservations/new", (req, res) => {
  ...
  if (reqDate > currentDate) {
    ...
    const costPerHead = instance.find((item) => item.id == req.body.adventure).costPerHead;
    const adventureName = instance.find((item) => item.id == req.body.adventure).name;
...
// ------------------------------------------
// ✅ [POST] Add a random new adventure
// ------------------------------------------
